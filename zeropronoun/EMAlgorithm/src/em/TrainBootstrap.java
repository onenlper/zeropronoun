...
public class TrainBootstrap {
	...
	static SuperviseFea zeroFea;
	static OvertPronounCorefFea overtFea;

	static List<Datum<String, String>> trainingData = new ArrayList<Datum<String, String>>();

	static ArrayList<String> yasmetOvert_ZeroFea = new ArrayList<String>();
	static ArrayList<String> yasmetZero_ZeroFea = new ArrayList<String>();
	
	static ArrayList<String> yasmetZero_OvertFea = new ArrayList<String>();
	static ArrayList<String> yasmetOvert_OvertFea = new ArrayList<String>();
	
	static ArrayList<String> yasmetZeroTest_OvertFea = new ArrayList<String>();
	...
	public static void extractGroups(CoNLLPart part) {
		...
		for (int i = 0; i < allMentions.size(); i++) {
			...
			if (m.isAZP
				...
				StringBuilder mrYSBZero = new StringBuilder();
				StringBuilder mrYSBOvert = new StringBuilder();
				
				StringBuilder mrYSBZeroForOvertFeatureTest = new StringBuilder();
				...
				for (int k = 0; k < ants.size(); k++) {
					...
					if (coref) {
						if (!findCoref) {
							mrYSBZero.insert(0, k + " @ ");
							mrYSBOvert.insert(0, k + " @ ");
							findCoref = true;
						}
						rank = 1;
					}
					...
					zeroFea.configure("", "", "", "", context, ant, m, part);
					String zeroFeaStr = zeroFea.getSVMFormatString();
					mrYSBZero.append(getYamset(coref, zeroFeaStr, corefCount));
					
					overtFea.configure("", "", "", "", context, ant, m, part);
					String overtFeaStr = overtFea.getSVMFormatString();
					mrYSBOvert.append(getYamset(coref, overtFeaStr, corefCount));
					
					for(String pro : EMUtil.pronounList) {
						m.animacy = EMUtil.getAnimacy(pro);
						m.gender = EMUtil.getGender(pro);
						m.number = EMUtil.getNumber(pro);
						m.person = EMUtil.getPerson(pro);
						overtFeaStr = overtFea.getSVMFormatString();
						mrYSBZeroForOvertFeatureTest.append(getYamset(coref, overtFeaStr, corefCount));
					}
					
					
//					if (corefCount > 0) {
						if (m.isAZP) {
							svmRanksAZP.add(getSVMRank(rank, zeroFeaStr));
						} else {
							svmRanks.add(getSVMRank(rank, zeroFeaStr));
						}
//					}
...
//								break;
						...
						for(int c=0;c<cluster.size();c++) {
							...
							zeroFea.configure("", "", "", "", context, cant, m, part);
							zeroFeaStr = zeroFea.getSVMFormatString();
							
							String tks[] = zeroFeaStr.split("\\s+");
							for(String tk : tks) {
...
//					System.out.println(sb.toString());
				...
				for (int k = ants.size(); k < 100; k++) {
					mrYSBZero.append("@ 0 NOCLASS 1 # ");
					mrYSBOvert.append("@ 0 NOCLASS 1 # ");
					for(int j=0;j<10;j++) {
						mrYSBZeroForOvertFeatureTest.append("@ 0 NOCLASS 1 # ");
					}
				}
				...
				if (corefCount > 0) {
					if (m.isAZP) {
						yasmetZero_OvertFea.add(mrYSBOvert.toString().trim());
						yasmetZero_ZeroFea.add(mrYSBZero.toString().trim());
						
						yasmetZeroTest_OvertFea.add(mrYSBZeroForOvertFeatureTest.toString().trim());
						
						yasmetAZPCR.add(crYSB.toString().trim());
					} else {
						yasmetOvert_OvertFea.add(mrYSBOvert.toString().trim());
						yasmetOvert_ZeroFea.add(mrYSBZero.toString().trim());
...
//		String svm = superFea.getSVMFormatString();
	...
	public static void main(String args[]) throws Exception {
		...
		overtFea = new OvertPronounCorefFea(true, "overtFea");
		zeroFea = new SuperviseFea(true, "zeroFea");
		zeroFea.plusNumberGenderPersonAnimacy = false;
		...
		Common.outputLines(yasmetOvert_ZeroFea, "yasmetOvert_ZeroFea");
		Common.outputLines(yasmetZero_ZeroFea, "yasmetZero_ZeroFea");

		Common.outputLines(yasmetOvert_OvertFea, "yasmetOvert_OvertFea");
		Common.outputLines(yasmetZero_OvertFea, "yasmetZero_OvertFea");

		Common.outputLines(yasmetZeroTest_OvertFea, "yasmetZeroTest_OvertFea");
