...
public class ApplyMaxEntJAIR {
        ...
        public ApplyMaxEntJAIR(String folder) {
                this.folder = folder;
                try {
...
//        static HashSet<String> allV = new HashSet<String>();
//        static HashSet<String> allO = new HashSet<String>();
//        static int ONum = 0;
...
//                        	continue;
                        ...
                        double[] probNum = ApplyMaxEntJAIR.selectRestriction("number", all,
                                        v);
                        double[] probNum2 = ApplyMaxEntJAIR.selectRestriction2("number", all,
                                v, o);                
                                       

                        all = EMUtil.Gender.values().length - 1;
                        double[] probGen = ApplyMaxEntJAIR.selectRestriction("gender", all,
                                        v);
                        double[] probGen2 = ApplyMaxEntJAIR.selectRestriction2("gender", all,
                                v, o);

                        all = EMUtil.Person.values().length;
                        double[] probPer = ApplyMaxEntJAIR.selectRestriction("person", all,
                                        v);
                        double[] probPer2 = ApplyMaxEntJAIR.selectRestriction2("person", all,
                                v, o);

                        all = EMUtil.Animacy.values().length - 1;
                        double[] probAni = ApplyMaxEntJAIR.selectRestriction("animacy",
                                        all, v);
                        double[] probAni2 = ApplyMaxEntJAIR.selectRestriction2("animacy",
                                all, v, o);
                        // TODO
                        ...
                        if (antCount != 0 && (mode == classify || mode == load)) {
                                ...
                                ILPJAIR ilp = new ILPJAIR(numberOfAnt, probAnt, probNum, probGen,
                                                probPer, probAni, probNum2, probGen2,
                                                probPer2, probAni2);
                                try {
                                        antIdx = ilp.execute();
...
//                                 return ret;
        ...
        public static double[] selectRestriction2(String attri, int all, String v, String o) {
            double ret[] = new double[all];
            if (o==null) {
            	return ret;
            }
            ...
            HashMap<Integer, Integer> map = null;
            if (attri.equals("number")) {
                    map = Context.svoStat.numberStat2.get(vo);
            } else if (attri.equals("gender")) {
                    map = Context.svoStat.genderStat2.get(vo);
            } else if (attri.equals("person")) {
                    map = Context.svoStat.personStat2.get(vo);
            } else if (attri.equals("animacy")) {
                    map = Context.svoStat.animacyStat2.get(vo);
            } else {
                    Common.bangErrorPOS("No Such Attri");
            }
            if (map == null) {
        ...
        public static void evaluate(ArrayList<ArrayList<Mention>> zeroses,
                ...
                String para = ILPJAIR.a_num2 + " " + ILPJAIR.b_gen2 + " " + ILPJAIR.c_per2 + " "
                                + ILPJAIR.d_ani2;
...
//                        Common.outputHashSet(allV, "allV");
//                        Common.outputHashSet(allO, "allO");
//                        System.out.println(ONum);
                ...
                // ILPJAIR.a_num = para[a];
                // ILPJAIR.b_gen = para[b];
                // ILPJAIR.c_per = para[c];
                // ILPJAIR.d_ani = para[d];
                // // while(true) {
                ...
                for (int i=0;i<paras.length;i++) {
                        ...
                        ILPJAIR.a_num2 = Double.parseDouble(tks[0]);
                        ILPJAIR.b_gen2 = Double.parseDouble(tks[1]);
                        ILPJAIR.c_per2 = Double.parseDouble(tks[2]);
                        ILPJAIR.d_ani2 = Double.parseDouble(tks[3]);
                        System.err.println(i + "/" + paras.length);
                        run(args[0]);
                        // cc
                        // // if(ILPJAIR.c_per>ILPJAIR.d_ani && ILPJAIR.c_per>ILPJAIR.b_gen)
                        //
                        // if (para[a] <= 0.04 && para[a] > 0 && para[b] <= 0.01
                        // && para[c] >= 0.04 && para[d] >= 0.02) {
//                      if (ILPJAIR.a_num + ILPJAIR.b_gen + ILPJAIR.c_per + ILPJAIR.d_ani == 0
//                                      || ILPJAIR.a_num * ILPJAIR.b_gen * ILPJAIR.c_per * ILPJAIR.d_ani != 0)
        ...
        public static void run(String folder) {
                EMUtil.train = false;
                ApplyMaxEntJAIR test = new ApplyMaxEntJAIR(folder);
                test.test();
