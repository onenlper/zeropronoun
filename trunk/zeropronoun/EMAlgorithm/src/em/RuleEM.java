...
//					if(cand.s==zero.s && cand.gram==Grammatic.object && cand.end+2==zero.start && 
...
//						continue;
					...
					if (fracContextCount.containsKey(context.toString())) {
						p_context = fracContextCount.get(context.toString())
								/ contextPrior.get(context.toString());
					}
...
//					System.out.println(zero.antecedent.msg);
	...
	public static boolean findBest(Mention zero, ArrayList<Mention> cands) {
		...
		for (int i = 0; i < cands.size(); i++) {
			Mention cand = cands.get(i);
			if(cand.gram==EMUtil.Grammatic.subject && cand.MI>0 && cand.s==zero.s) {
				findBest = true;
				cand.isBest = true;
				break;
			}
